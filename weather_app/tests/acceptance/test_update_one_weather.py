...
class TestWeather:
    ...
    def test_updates_a_weather(self) -> None:
        ...
        update_response = client.put(
            f"/api/v1/weather/{weather_id}", json={"temperature": 100, "city": "Roma"}
        )
    ...
    def test_returns_not_found(self) -> None:
        ...
        response = client.put(
            ...
            json={"temperature": TestData.ANY_TEMPERATURE, "city": TestData.ANY_CITY},
        )

        weather_json = response.json()
        expect(response.status_code).to(be(status.HTTP_404_NOT_FOUND))
        expect(weather_json["detail"]).to(equal(f"Weather {any_weather_id} not found"))

    def test_returns_bad_request(self) -> None:
        invalid_city = ""
        payload = {"temperature": TestData.ANY_TEMPERATURE, "city": invalid_city}
        response = client.put(
            f"/api/v1/weather/{TestData.ANY_WEATHER_ID}", json=payload
        )
